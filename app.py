...
# --- INTERFAZ ---
st.title("🐦 Turaco PrestaShop Assistant - v4.5")
st.markdown("> **📌 Nota:** Los ficheros de **Imágenes** y **Categorías** deben ser **Excel (.xlsx)**.")

with st.expander("📋 Guía de Columnas Requeridas"):
    c1, c2, c3 = st.columns(3)
    with c1:
        st.info("**1. Keepa (XLSX)**")
        st.write("- `ASIN`, `Título`\n- `Subcategoría`, `EAN`\n- `Features` o `Feature 1, 2...` (Para la descripción)")
    with c2:
        st.info("**2. Imágenes (XLSX)**")
        st.write("- `Reference` / `SKU` (Cruce)\n- `URLs` en columnas siguientes")
    with c3:
        st.info("**3. Otros**")
        st.write("- **Plan:** `Solo MAIN` ahora incluido.")
...
with tab2:
    ...
    if st.session_state.df_revisado is None:
        st.info("⚠️ Pasa por la Fase 1 primero.")
    else:
        c1, c2 = st.columns(2)
        ...
        if all([f_keepa, f_img, f_cats]):
            if st.button("🪄 Generar Excel"):
                try:
                    ...
                    df_m = pd.merge(st.session_state.df_revisado, df_k.drop_duplicates(subset=[buscar_columna(df_k, ['asin'])]), left_on='ASIN', right_on=buscar_columna(df_k, ['asin']), how='inner')
                    ...
                    # --- CONCATENACIÓN DE DESCRIPCIÓN (FIX KEEPA) ---
                    # Busca columnas que contengan 'feature' (nombre Keepa) o 'característica'
                    cols_desc = [c for c in df_m.columns if any(p in str(c).lower() for p in ['feature', 'característica'])]
                    if cols_desc:
                    ...
                    # Relleno de campos fijos
                    for c, v in {'Price tax included':"999", 'Tax rules ID':"1", 'Supplier':"Cecotec", 'Manufacturer':"Cecotec", 'Quantity':"0", 'Show price (0 = No, 1 = Yes)':"1", 'Condition':"new"}.items():
                        final[c] = v
        ...
        if 'df_final_generado' in st.session_state and st.session_state.df_final_generado is not None:

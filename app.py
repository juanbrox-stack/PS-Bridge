...
# --- INTERFAZ ---
st.title("🐦 Turaco PrestaShop Assistant - v4.8")
st.markdown("> **📌 Nota importante:** Usa archivos **Excel (.xlsx)** para evitar errores de saltos de línea y duplicados.")

# --- GUÍA DE COLUMNAS ACTUALIZADA (NUEVO ORDEN) ---
with st.expander("📋 Guía de Columnas Requeridas (Keepa Actualizado)"):
    c1, c2, c3 = st.columns(3)
    with c1:
        st.info("**1. Keepa (XLSX) - Nuevo Orden**")
        st.write("""
        1. `ASIN`
        2. **`Modelo` (SKU)**
        3. **`Título`**
        4. `Categorías: Subcategoría`
        5-9. `Característica 5` a `1`
        10. `EAN`
        """)
    with c2:
        st.info("**2. Plan de Lanzamiento**")
        st.write("- `SKU` | - `Estado` | - `Notas` (Incluye 'Solo MAIN')")
    with c3:
        ...
        if st.button("🚀 Iniciar Cruce"):
            try:
                ...
                if not df_final.empty:
                    ...
                    df_viz = df_final[['sku_norm', c_name, c_asin, 'notas']].copy()
                    df_viz.rename(columns={c_asin: 'ASIN', c_name: 'item-name', 'sku_norm': 'SKU'}, inplace=True)
        ...
        if all([f_keepa, f_img, f_cats]):
            if st.button("🪄 Generar Excel Final"):
                try:
                    ...
                    # Usa 'título' o la columna 3 (index 2)
                    ...
                    # El SKU ahora viene de 'Modelo' en Keepa o del cruce previo
                    ...
                    st.success("✅ Excel generado correctamente con el nuevo orden de Keepa.")
